...
# -*- coding: utf-8 -*-
"""
トクバイ最新チラシ（表裏など複数面対応）を取得し、PNG と PDF を作って Dropbox に保存。

必要:
  - GitHub Secrets: DROPBOX_ACCESS_TOKEN
任意:
  - 環境変数 FALLBACK_PRINT_URL（検出失敗時の保険）

編集ポイント:
  - STORE_URL（店舗トップURL）
  - DROPBOX_BASE_DIR（Dropbox保存先ルート）
"""
import os
import re
import time
...
HEADERS = {
    "User-Agent": "Mozilla/5.0 (compatible; TokubaiSaver/1.2)",
...
def build_print_url_from_id(leaflet_id: str) -> str:
    """https://tokubai.co.jp/<chain>/<store>/leaflets/{id}/print"""
...
def enumerate_print_variants(base_print_url: str) -> List[str]:
    """表裏/複数面に対応するため、想定バリエーションを列挙"""
    variants: List[str] = [base_print_url]
    # 代表的なクエリ（2面/3面や裏面表現）
...
def ensure_folder(dbx: dropbox.Dropbox, path: str) -> None:
    """フォルダ作成。既存フォルダなら衝突を検知して無視して続行する。"""
    try:
        ...
        try:
            ...
            if hasattr(err, "is_path") and err.is_path():
                p = err.get_path()
                if hasattr(p, "is_conflict") and p.is_conflict():
                    return  # 既に存在 → OK
        except Exception:
            pass
        # 文字列でも保険判定
