...
トクバイ チラシ自動保存スクリプト
 ...
 - 表裏/複数面対応
 - 重複画像はMD5で排除
 - Dropboxに /Tokubai/店舗/日付/leaflet_id/ 以下で保存
"""
...
MAX_LEAFLETS = 3   # 1日に複数ある場合、最大で何本保存するか
...
def download_leaflet_images(print_url: str) -> List[str]:
    ...
    for u in enumerate_print_variants(print_url):
        ...
        for url in sorted(imgs):
            ...
            seen_hashes.add(h)
            fname = f"{today}_{leaflet_id}_p{len(saved)+1}.png"
...
def main():
    ...
    for purl in print_urls:
        ...
        pdf_path = None
        if imgs:
            pdf_path = os.path.join(SAVE_DIR, f"{today}_{leaflet_id}.pdf")
