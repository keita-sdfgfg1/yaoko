...
def _extract_leaflet_print_from_html(html: str, base: str) -> Optional[str]:
    """HTMLから /leaflets/{id}/print を最優先で拾い、無ければ /leaflets/{id} を拾って/printを付ける。
       `/leaflets/{id}` のように“店舗パスが省略”されている場合は STORE_PATH を前置して補正する。
    """
    soup = BeautifulSoup(html, "html.parser")

    # 1st: 直接 /<chain>/<store>/leaflets/{id}/print を拾う（短い /leaflets/{id}/print も補正）
    for a in soup.find_all("a", href=True):
        href = a["href"]
        m = PRINT_RE.search(href)
        if m:
            if href.startswith("/leaflets/"):  # 店舗パス省略を補正
                return build_print_url_from_id(m.group(1))
            return urljoin(base, href)

    # 2nd: /leaflets/{id} を拾って /print を付ける（短いパスも補正）
    for a in soup.find_all("a", href=True):
        href = a["href"]
    ...
    # 3rd: meta/link からID抽出して店舗パスで組み立て
